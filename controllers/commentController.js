...
import {Post,Comment} from '../model/index.js';
...
function save(req, res) {
    ...
    if(validationResponse !== true){
      ...
      Post.findByPk(req.body.post_id).then(post => {
        if(post === null){
            res.status(404).json({
...
function update(req, res){
    ...
    Comment.update(updatedComment, {where: {id:id, userId: userId}}).then(result => {
        res.status(200).json({
...
function destroy(req, res){
    ...
    const userId = 1;
      ...
      Comment.destroy({where:{id:id, userId:userId}}).then(result => {
        res.status(200).json({
